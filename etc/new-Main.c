...
int main(int argc, char **argv) {
  ...
  /* - O_RDWR opens dir for reading and writing
     - O_NOCTTY excludes terminal input requirements.
     - O_NDELAY immediately opens the serial port.
   */
  file_descr = open(dir, O_RDWR | O_NOCTTY | O_NDELAY);
  ...
  for (t = 0; t < intv; ++t) {
	...
	case 3:
	  ...
	  fprintf(stdout, "Amps = %.4lf, ", amps);
	  tot_amps += amps;
	  break;
	case 4:
	  volts = atoi(word) / 10.0;
	  fprintf(stdout, "Volts = %.4lf, ", volts);
	  tot_volts += volts;
	  break;
	case 5:
	  watts = atoi(word) / 10.0;
	  tot_watts += watts;
	  fprintf(stdout, "Watts = %.4lf, ", atoi(word) / 10.0);
	  fprintf(stdout, "Energy = %.4lf", tot_watts);
	  break;
	default:
	  break;
	}
